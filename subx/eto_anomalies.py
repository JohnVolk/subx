...
netCDFs that are pre-downloaded are used, using the most recent download date. Plots of the 4 week anomalies are saved as a final result.
...
baseline_dir = 'ETo_baseline' # directory with baseline anomalies
...
#### Collect ETo input netCDFs- assumes downloaded to local "subx/forecasts/" 
...
for v in vars_downloaded:
    ...
    for m in models_per_var:
        ...
        dates = [filename_date_re.search(f.name).group(1) for f in var_files]
...
baseline_path = Path(
    f'{baseline_dir}/eto_climo_weekly_v2_{baseline_model_name.get(
        model, model)}'
)
